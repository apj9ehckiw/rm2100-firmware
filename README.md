...
**当前固件版本：v2.6.0**（刷入后 `cat /etc/campus-fix-version` 查询）
...
1. **构建**：本仓库已配好 Actions。进 **Actions → Build RM AC2100 OpenWrt
   firmware → Run workflow**，默认参数（24.10.2 + LuCI + Argon 主题 + 简体中文 + v2.6.0）直接 Run，
   约 3-5 分钟出包。可调输入：
...
```
cat /etc/campus-fix-version        # 应显示 2.6.0
