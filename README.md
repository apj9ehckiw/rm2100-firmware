...
**当前固件版本：v2.10.2**（刷入后 `cat /etc/campus-fix-version` 查询；LuCI 页脚也显示 `campus-fix vX.Y.Z`）
...
```
cat /etc/campus-fix-version        # 应显示 2.10.2
...
| v2.10.2 | 合并保留 v2.10.1 的认证会话、curl、IP-ID 与 MAC OUI 策略；修复计时函数分秒前导零、封禁记录跨重启保存（原子写入 /etc，兼容旧 /tmp 记录）、防火墙渲染失败检查与查询参数提取；修正 curl 输出参数为 -o -、修复 ucode 全局正则匹配调用；新增构建前回归测试及同版本原生 ucode 冒烟测试。 |
