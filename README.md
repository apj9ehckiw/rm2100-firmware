...
当前源码另包含尚未重新打包的审查修复：RPC 参数 shell 转义、分秒前导零计算、
Portal 查询参数提取、fw4guard 启动入口、随机 MAC 冒号格式及封禁记录跨重启保存。
已有 `.bin` 不包含这些源码修复，需要重新构建后刷入。

## 源码回归验证

运行 `python3 tests/test_review_fixes.py`，依赖 Python 3.10+、Node.js，以及
BusyBox ash（优先）或 dash；Windows 可使用 Git for Windows 自带的 dash。
构建工作流会在打包前运行同一测试。测试覆盖特殊字符命令参数、全部 3,600 种分秒组合、
启动守卫成功/失败路径、MAC 格式、封禁记录迁移/恢复/过期；所有写入均在临时目录内，
不会连接校园网或真实路由器。RPC 使用 Node.js 兼容层，不能替代目标设备上的原生 ucode 验证。
...
| 触发「代理行为」封禁 | v2.9.0 起封禁期完全静默且**跨重启持久化**（当前源码使用 `/etc/campus-auth.banuntil`，原 v2.9.0/v2.9.1 的 `/tmp` 路径仅支持进程重启），到期自动恢复；期间 LuCI「立即认证」也会拒绝并提示到期时间——**请勿反复手动尝试**（同 MAC 重复认证会加重标记）。封禁结束后若仍频繁触发：先看 `nft list chain inet fw4 campus_synrate` drop 计数（聚合速率是否常态超限），再确认终端是否开着代理/VPN 客户端 |
